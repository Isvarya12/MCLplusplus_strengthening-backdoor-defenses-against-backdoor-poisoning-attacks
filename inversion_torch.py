...
#     def generate(self, data_loader, target, attack_size=100, trigger_type='random'):
...
#                     trigger_pattern = torch.rand(self.pattern_shape).to(self.device)
#                 elif trigger_type == 'constant':
#                     trigger_pattern = torch.ones(self.pattern_shape).to(self.device)
#                 elif trigger_type == 'gradient':
#                     trigger_pattern = torch.zeros(self.pattern_shape).to(self.device)
#                     trigger_pattern[:, 0, :, :] = 1.0  # Modify the trigger based on your requirements
#                 else:
#                     trigger_pattern = torch.rand(self.pattern_shape).to(self.device)

#                 x_adv = torch.clamp(x_batch + pattern_pos + pattern_neg + trigger_pattern, min=0.0, max=self.clip_max)
#                 x_adv = self.normalize(x_adv)
#                 y_batch = torch.full((x_batch.size(0),), target, dtype=torch.long).to(self.device)
...
class PixelBackdoor:
    def __init__(self,
        ...
        # Default normalization function (identity function)
